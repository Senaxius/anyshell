...
make -C $DIR clean && make -j8 -C $DIR &>/dev/null || echo "could not compile anyshell, using already compiled version..." && cp /opt/anyshell/.backup/anyshell-precomp $DIR/anyshell && anyshell
echo "---------------------------done!------------------------------"
